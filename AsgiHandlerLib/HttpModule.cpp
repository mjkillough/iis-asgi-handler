...
)
{
    ...
    // Freed by IIS when the IHttpContext is destroyed, via
    // StoredRequestContext::CleanupStoredContext()
    ...
    if (upgrade_header_length > 0) {
        ...
        request_handler = new WsRequestHandler(
            m_response_pump, m_channels, logger, http_context
        );
    } else {
    ...
    http_context->GetModuleContextContainer()->SetModuleContext(
        request_handler, m_factory.module_id()
    );
